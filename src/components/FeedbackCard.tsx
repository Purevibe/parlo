  ...
  return (
    <div className="space-y-3 transition-all duration-300">
      ...
      {corrections && corrections.length > 0 && (
        <div className="bg-slate-900/90 border border-rose-900/50 rounded-2xl p-4 shadow-xl space-y-3">
          <div className="flex items-center space-x-2 text-rose-400">
            <AlertTriangle className="w-4 h-4 shrink-0" />
            <h4 className="text-xs font-bold uppercase tracking-wider font-display">
              Grammar Breakdown & Corrections
            </h4>
          </div>

          <div className="space-y-2">
            {corrections.map((corr, idx) => (
              <div key={idx} className="bg-slate-950/80 rounded-xl p-3 border border-rose-900/30 text-xs space-y-1.5">
